...
START_URL = f"{BASE_URL}/alegeri-prezidentiale"
HEADERS = {"User-Agent": "Mozilla/5.0"}

def get_articles_from_mediafax():
    ...
    article_links = soup.select("h2 a[href^='/politic/']")  # Selector pentru articole
...
if __name__ == "__main__":
    all_articles = get_articles_from_mediafax()
    df = pd.DataFrame(all_articles)
    df.drop_duplicates(subset="source", inplace=True)
    df["id"] = range(1, len(df) + 1)
    df.to_csv("../data/stiri_mediafax_alegeri.csv", index=False)
    print(f"\n✅ Am salvat {len(df)} articole în ../data/stiri_mediafax_alegeri.csv")
